...
// ---------------------------------------------------------------------------
// Leitura dos botões:
//   - SW0: muda o período do timer (igual antes)
//   - SW1: muda o efeito atual (LOOPBACK -> FLANGER -> TREMOLO -> REVERB -> ...)
// ---------------------------------------------------------------------------
void checkSwitch(void)
{
    static Uint8 lastEffectButtonState = 1;  // estado anterior de SW1 (p/ borda)
    Uint8 sw0Raw;
    Uint8 sw1Raw;
    ...
    // --- SW0: controla o timer, com lógica antiga (pressiona -> alterna período)
    if (sw0Raw == 0)
    {
    ...
    // --- SW1: muda o efeito (detecção de borda 1 -> 0, sem debounce pesado)
